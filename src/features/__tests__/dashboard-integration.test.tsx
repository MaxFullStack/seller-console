...
describe('Dashboard Data Integration', () => {
  ...
  describe('Real-time Metrics Calculation from Live Data', () => {
    it('should calculate accurate metrics from actual repository data', async () => {
      ...
      const leadInputs: CreateLeadInput[] = [
        { name: 'Lead 1', company: 'Company 1', email: 'lead1@co1.com', source: 'web', score: 80, status: 'new' },
        { name: 'Lead 2', company: 'Company 2', email: 'lead2@co2.com', source: 'email', score: 90, status: 'qualified' },
        { name: 'Lead 3', company: 'Company 3', email: 'lead3@co3.com', source: 'referral', score: 70, status: 'qualified' },
        { name: 'Lead 4', company: 'Company 4', email: 'lead4@co4.com', source: 'social', score: 60, status: 'contacted' },
        { name: 'Lead 5', company: 'Company 5', email: 'lead5@co5.com', source: 'phone', score: 95, status: 'unqualified' },
      ];
      ...
      const opportunityInputs: CreateOpportunityInput[] = [
        { name: 'Opp 1', stage: 'closed-won', amount: 50000, accountName: 'Account 1', leadId: createdLeads[1].id },
        { name: 'Opp 2', stage: 'closed-won', amount: 75000, accountName: 'Account 2', leadId: createdLeads[2].id },
        { name: 'Opp 3', stage: 'closed-lost', amount: 40000, accountName: 'Account 3', leadId: createdLeads[3].id },
        { name: 'Opp 4', stage: 'proposal', amount: 60000, accountName: 'Account 4' },
        { name: 'Opp 5', stage: 'negotiation', amount: 85000, accountName: 'Account 5' },
      ];
      ...
      // Test leads metrics
      const { result: leadsMetricsResult } = renderHook(() => useLeadsMetrics());
      
      expect(leadsMetricsResult.current.totalLeads).toBeGreaterThanOrEqual(5);
      expect(leadsMetricsResult.current.qualifiedLeads).toBeGreaterThanOrEqual(2);
      expect(leadsMetricsResult.current.newLeads).toBeGreaterThanOrEqual(1);
      expect(leadsMetricsResult.current.contactedLeads).toBeGreaterThanOrEqual(1);
      expect(leadsMetricsResult.current.unqualifiedLeads).toBeGreaterThanOrEqual(1);
      ...
      const { result: opportunitiesMetricsResult } = renderHook(() => useOpportunitiesMetrics());
      
      expect(opportunitiesMetricsResult.current.totalOpportunities).toBeGreaterThanOrEqual(5);
      expect(opportunitiesMetricsResult.current.closedWonCount).toBeGreaterThanOrEqual(2);
      expect(opportunitiesMetricsResult.current.totalRevenue).toBeGreaterThanOrEqual(125000); // 50k + 75k
      expect(opportunitiesMetricsResult.current.activeOpportunities).toBeGreaterThanOrEqual(2);
      ...
      const { result: overallMetricsResult } = renderHook(() => useOverallMetrics());
      
      expect(overallMetricsResult.current.revenuePerLead).toBeGreaterThan(15000); // 125k+ / 8+ leads
      expect(overallMetricsResult.current.lastUpdated).toBeDefined();
      expect(overallMetricsResult.current.leads.totalLeads).toBe(leadsMetricsResult.current.totalLeads);
      expect(overallMetricsResult.current.opportunities.totalRevenue).toBe(opportunitiesMetricsResult.current.totalRevenue);
    });
    ...
    it('should update metrics dynamically when data changes', async () => {
      ...
      expect(opportunitiesMetricsResult.current.totalOpportunities).toBe(initialOpportunitiesCount + 1);
      expect(opportunitiesMetricsResult.current.totalRevenue).toBe(initialRevenue + 100000);
      expect(opportunitiesMetricsResult.current.closedWonCount).toBeGreaterThan(0);
  ...
  describe('Cross-Domain Metrics Accuracy', () => {
    it('should calculate revenue per lead correctly with real conversion data', async () => {
      ...
      const conversions = [
        { leadId: qualifiedLeads[0].id, amount: 80000, stage: 'closed-won' as const },
        { leadId: qualifiedLeads[1].id, amount: 120000, stage: 'closed-won' as const },
        { leadId: qualifiedLeads[2].id, amount: 60000, stage: 'closed-lost' as const },
        // qualifiedLeads[3] remains unconverted
      ];

      const opportunities: Opportunity[] = [];
      for (const conversion of conversions) {
        const leadIndex = qualifiedLeads.findIndex(lead => lead.id === conversion.leadId);
      ...
      expect(overallMetricsResult.current.opportunities.totalRevenue).toBeGreaterThanOrEqual(200000);
      ...
      expect(overallMetricsResult.current.opportunities.opportunityConversionRate).toBeGreaterThan(0);
      expect(overallMetricsResult.current.opportunities.winRate).toBeGreaterThan(50); // 2/3 = 67%
    });
    ...
    it('should handle edge cases in cross-domain calculations', async () => {
      ...
      expect(overallMetricsResult.current.opportunities.totalRevenue).toBeGreaterThanOrEqual(0);
      expect(overallMetricsResult.current.opportunities.totalPipelineValue).toBeGreaterThan(0);
      expect(overallMetricsResult.current.opportunities.activeOpportunities).toBeGreaterThan(0);
    });
  });
  ...
  describe('Store Synchronization', () => {
    it('should maintain data consistency between repositories and dashboard store', async () => {
      ...
      const repositoryOpportunity = await opportunityRepository.create(opportunityInput);
      ...
      expect(storeState.leads.some(lead => lead.id === repositoryLead.id)).toBe(true);
      expect(storeState.opportunities.some(opp => opp.id === repositoryOpportunity.id)).toBe(true);

      const storeLeadCopy = storeState.leads.find(lead => lead.id === repositoryLead.id);
      const storeOpportunityCopy = storeState.opportunities.find(opp => opp.id === repositoryOpportunity.id);
      ...
      expect(metricsResult.current.leads.totalLeads).toBe(storeState.leads.length);
      expect(metricsResult.current.opportunities.totalOpportunities).toBe(storeState.opportunities.length);
    });
    ...
    it('should handle lastUpdated timestamp correctly', async () => {
      ...
      expect(finalTimestamp?.getTime()).toBeGreaterThan(afterLeadsTimestamp?.getTime() || 0);
