...
describe('Lead Conversion Integration', () => {
  ...
  describe('Complete Lead to Opportunity Conversion Flow', () => {
    it('should successfully convert qualified lead to opportunity and remove lead', async () => {
      ...
      const createdOpportunity = await opportunityRepository.create(opportunityInput);
      ...
      expect(finalOpportunities.some(opp => opp.leadId === updatedLead.id)).toBe(true);
      
      const linkedOpportunity = finalOpportunities.find(opp => opp.leadId === updatedLead.id);
    ...
    it('should handle conversion failure gracefully', async () => {
      ...
      const unchangedLead = finalLeads.find(lead => lead.id === createdLead.id);
    ...
    it('should maintain data consistency across multiple conversions', async () => {
      ...
      for (const originalLead of createdLeads) {
        expect(finalLeads.some(lead => lead.id === originalLead.id)).toBe(false);
      }
      ...
      for (const { lead, opportunity } of conversions) {
        ...
        const linkedOpportunity = finalOpportunities.find(opp => opp.id === opportunity.id);
      ...
      expect(finalOpportunities.length).toBeGreaterThanOrEqual(conversions.length);
    });
  });
  ...
  describe('Lead Status Progression Integration', () => {
    it('should track complete lead journey through status changes', async () => {
      ...
      expect(remainingLeads.some(lead => lead.id === currentLead.id)).toBe(false);
      expect(allOpportunities.some(opp => opp.leadId === currentLead.id)).toBe(true);

      const convertedOpportunity = allOpportunities.find(opp => opp.leadId === currentLead.id);
    ...
    it('should handle unqualified leads appropriately', async () => {
      ...
      const unqualifiedLead = finalLeads.find(lead => lead.id === currentLead.id);
  ...
  describe('Cross-Repository Data Integrity', () => {
    ...
    it('should handle orphaned opportunity cleanup scenarios', async () => {
      ...
      const allOpportunities = await opportunityRepository.list();
      const orphanedOpportunity = allOpportunities.find(opp => opp.leadId === lead.id);
