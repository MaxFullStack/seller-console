...
import { Opportunity, CreateOpportunityInput, UpdateOpportunityInput } from '../model/opportunity';

export const useOpportunities = () => {
  const [opportunities, executeOpportunitiesOperation] = useAsyncOperation<Opportunity[]>([]);

  const loadOpportunities = useCallback(async () => {
    await executeOpportunitiesOperation(() => opportunityRepository.list());
  }, [executeOpportunitiesOperation]);

  const createOpportunity = useCallback(async (input: CreateOpportunityInput) => {
  ...
  const updateOpportunity = useCallback(async (input: UpdateOpportunityInput) => {
    await executeOpportunitiesOperation(async () => {
