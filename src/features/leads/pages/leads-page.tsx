...
export const LeadsPage = () => {
  ...
  const { selectedLead, isPanelOpen, selectLead, closeLead } =
    useLeadSelection();
  const { filters, filteredLeads, updateSearch, updateStatus, clearFilters } =
    useLeadFilters(leads.data || []);
  const {
    isOpen: isConvertDialogOpen,
    openDialog: openConvertDialog,
    closeDialog: closeConvertDialog,
  } = useConvertDialog();
  ...
  const handleConvertLead = useCallback(
    async (input: CreateOpportunityInput) => {
      ...
      try {
        ...
        toast.success(
          `Lead "${selectedLead.name}" successfully converted to opportunity!`,
        );
        closeConvertDialog();
        closeLead();
      } catch {
